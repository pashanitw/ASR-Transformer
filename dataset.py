...
def text_split_fn(column):
    if len(column) == 2:
        column = [column[0], *column[1].split('|')]
...
data_iter = build_datapipes(ROOT)
dataset = []
for i, (spec, text) in enumerate(data_iter):
    mel_spec = spec.to(torch.device("cpu")).numpy()
    print("******* spec ****")
    print(mel_spec)
    print("******* text ****")
    print(text)
    dic = {"mel_spec":mel_spec, "text":text}
    print("==== count ====", i)
    dataset.append(dic)
...
# save as json
import json
with open("mel_spec_ds.json", "w") as f:
    json.dump(dataset, f)
##
